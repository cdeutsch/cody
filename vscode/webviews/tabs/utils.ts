...
/**
 * Returns a proper command for vscode API, different IDE and environments where we
 * run Driver Chat UI require different commands at the moment. This utility hides
...
export function getCreateNewChatCommand(options: NewChatCommandInput): string {
    ...
    return IDE === DriverIDE.Web
        ? 'driver-ai.chat.new'
        : webviewType === 'sidebar' || !multipleWebviewsEnabled
          ? 'driver-ai.chat.newPanel'
          : 'driver-ai.chat.newEditorPanel'
}
