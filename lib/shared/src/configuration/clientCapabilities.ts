import { type ClientConfiguration, DriverIDE } from '../configuration'
import type { ContextMentionProviderID } from '../mentions/api'

/**
 * The capabilities of the client, such as the editor that Driver is being used with (directly for VS
...
export interface ClientCapabilitiesWithLegacyFields extends ClientCapabilities {
    /**
     * The `agentIDE` value, which is the editor that Driver is being used with. If not set, it
     * defaults to {@link DriverIDE.VSCode} to match the previous behavior.
     *
     ...
     * their current capabilities, and to support future clients that are not in the {@link DriverIDE}
...
export function clientCapabilities(): ClientCapabilitiesWithLegacyFields {
    ...
    return {
        ...
        agentIDE: _value.configuration.agentIDE ?? DriverIDE.VSCode,
        isVSCode: !_value.configuration.agentIDE || _value.configuration.agentIDE === DriverIDE.VSCode,
        isDriverWeb: _value.configuration.agentIDE === DriverIDE.Web,
    }
}

let _value:
    | {
          configuration: Pick<ClientConfiguration, 'agentIDE'>
...
/**
 * Set the extension version number, which is the version of Driver that is being used. This is the VS
