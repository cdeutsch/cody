 ...
 *
 * Also update MarkdownFromDriver's `ALLOWED_URI_REGEXP` if you change this.
 */
export const DRIVER_PASSTHROUGH_VSCODE_OPEN_COMMAND_ID = '_driver-ai.vscode.open'
...
function commandURIForVSCodeOpen(resource: URI, range?: RangeData): string {
    return `command:${DRIVER_PASSTHROUGH_VSCODE_OPEN_COMMAND_ID}?${encodeURIComponent(
        JSON.stringify([
            resource,
            {
                selection: range,
